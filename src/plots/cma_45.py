...
def get_coco_objects(model, processor, coco_val_dir, cache_file, max_images=None):
    ...
    for idx, img_path in enumerate(all_image_paths):
        ...
        prompt_text = get_text_prompt(model, prompt, img, processor)
...
def run_cma_coco_unit(
    ...
    source_prompt_texts = [
        get_text_prompt(model, source_prompt, img, processor) for img in source_images
    ]
    ...
    for i in range(len(target_images)):
        ...
        intervention_prompt_text = get_text_prompt(model, intervention_prompt, img, processor)
