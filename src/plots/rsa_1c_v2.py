...
from src.utils.tools import predict, get_num_hidden_layers, load_config, get_permutations, get_base_model_prompt
...
# model_id = "HuggingFaceM4/idefics2-8b"                      # Figure 19
...
def get_dynamic_token_indices(model: Any, processor: Any, colors: List[str], shapes: List[str], coords: List[tuple[int, int]], image: Image.Image, last_color: bool = True, do_shffule: bool = True):
    ...
    if last_color:
        ...
        text_prompt = get_base_model_prompt(model, prefix)
    else:
...
def main():
    model_id_list = [
        ("Qwen/Qwen2-VL-7B", "1c"),
        # ("Qwen/Qwen2.5-VL-3B", "14"),
        # ("Qwen/Qwen2.5-VL-7B", "15"),
        # ("Qwen/Qwen2.5-VL-32B", "16"),
