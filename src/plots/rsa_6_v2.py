...
# includes last object's color in prompt
...
def get_trial_data(model, processor, color_list, shape_list, num_trials):
    ...
    for i in range(num_trials):
        ...
        obj_indices, text_prompt = get_dynamic_token_indices(
            model, processor, colors=colors, shapes=shapes, coords=coords, image=img, last_color=True
        )
        ...
        pred_shape = ' '.join(pred).strip().lower()
        if len(pred) >= 1 and is_equiv(pred_shape, obj_indices[-1]['shape'], equiv_shapes):
            corr_trials += 1
        else:
            print(f"pred={pred}, target_shape={obj_indices[-1]['shape']}")
...
def main():
    ...
    filename = f"src/data/cma/entr_v2/fig_6_results.json"
    ...
    for model_id, model_label, fig_num in model_id_list:
        ...
        if model_label not in all_rsa_results:
            save_path = f"outputs/rsa/entr_v2/rsa_fig_{fig_num}_{model_name}"
    ...
    save_paths = [f"outputs/rsa/entr_v2/rsa_fig_7_{x}" for x in ['a','b','c']]
